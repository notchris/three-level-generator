...
import Block from '../../data/Blocks.json'
...
export default class Render {
    ...
    init () {
      ...
      this.camera.position.set(40, 140, 40);
      ...
      this.plane = new THREE.Mesh(
        new THREE.PlaneGeometry(80, 80, 8, 8),
        new THREE.MeshBasicMaterial({
          wireframe: true
        })
      )
      this.plane.visible = false;
      this.plane.geometry.rotateX(-Math.PI * 0.5)
      this.plane.geometry.translate(40, 40, 40)
      this.scene.add(this.plane);
      console.log(this.plane.geometry.vertices.length)
    ...
    dropBalls () {
        for (let i = 0; i < this.plane.geometry.vertices.length; i += 1) {
          ...
          mesh.position.copy(this.plane.geometry.vertices[i])
          
          let sphereShape = new CANNON.Sphere(0.5);
          let sphereBody = new CANNON.Body({ mass: 10 });
          ...
          this.ballBodyList.push(sphereBody);
          this.ballMeshList.push(mesh);
        }
    }
    ...
    updatePhysics () {
      if (this.clock) {
          ...
          this.bodyList.forEach((b, i) => {
            b.position.copy(this.meshList[i].position);
            b.quaternion.copy(this.meshList[i].quaternion);
            if (this.meshList[i].isModel) {
              this.meshList[i].rotation.y += 0.01;
            }
          })
  
          this.ballMeshList.forEach((m, i) => {
            m.position.copy(this.ballBodyList[i].position);
            m.quaternion.copy(this.ballBodyList[i].quaternion);
            if (this.ballBodyList[i].position.y < -100) {
              this.world.removeBody(this.ballBodyList[i])
              this.scene.remove(this.ballMeshList[i])
            }
        });
      }
   }
